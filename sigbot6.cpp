/*
 * AlgoTrade 2026 — Competition Bot (C++20)  [sigbot6]
 *
 * Four strategies:
 *   1. ETF arbitrage — PICNIC_BASKET style: weighted fair value
 *      fair = Σ(quantities[i] × price_i), hedge legs proportional to qty
 *   2. Safe-haven spread (ETFA/B − ETFSH mean-reverts around rolling μ)
 *   3. Sector pair z-arb (stock vs sector-peer-mean, correlation-weighted)
 *   4. Cross-venue same-ticker arbitrage
 *
 * Build:
 *   g++ -std=c++20 -O2 -o bot sigbot6.cpp -lpthread
 */
...
struct Params {
    ...
    // ─── ETF arbitrage (PICNIC_BASKET style) ────────────────────────
    // fair_value(1 ETF) = Σ(quantities[i] × mid_price_i)
    // Trade when |ETF_market_price − fair| ≥ EDGE_CENTS.
    // Hedge: BUY K ETFs → SELL quantities[i]*K of each constituent.
    static constexpr int  ETF_EDGE_CENTS    = 20;
    static constexpr int  ETF_EXIT_CENTS    = 5;
    static constexpr int  ETF_HEDGE_UNIT    = 1;     // K — base number of ETF lots
    static constexpr int  ETF_POS_CAP       = 60;    // max ETF qty per (etf, venue)
    static constexpr int  ETF_COOLDOWN_MS   = 250;

    // ─── Safe-haven spread ──────────────────────────────────────────
    static constexpr int  SH_HISTORY           = 600;
    static constexpr int  SH_MIN_SAMPLES       = 100;
    static constexpr int  SH_ENTRY_Z100        = 250;
    static constexpr int  SH_EXIT_Z100         = 50;
    static constexpr int  SH_STOP_Z100         = 600;
    static constexpr int  SH_STOP_COOLDOWN_MS  = 30'000;
    static constexpr int  SH_SIZE              = 10;
    static constexpr int  SH_POS_CAP           = 40;
    static constexpr int  SH_REBAL_MS          = 250;

    // ─── Sector pair z-arb ──────────────────────────────────────────
    static constexpr int  SECPAIR_HISTORY          = 600;
    static constexpr int  SECPAIR_MID_HIST         = 300;
    static constexpr int  SECPAIR_CORR_MIN_SAMPLES = 100;
    static constexpr int  SECPAIR_MIN_SAMPLES      = 150;
...
// ════════════════════════════════════════════════════════════════════
//  ETF definitions — PICNIC_BASKET style
//
//  quantities[i] = number of constituent[i] shares that replicate 1 ETF share.
//  fair_value(1 ETF) = Σ(quantities[i] × mid_price_i)
//
//  Hedge for K ETFs:
//    BUY  K ETFs → SELL quantities[i]*K of each constituent
//    SELL K ETFs → BUY  quantities[i]*K of each constituent
//
//  *** Update quantities from the competition rulebook if they differ! ***
//  Default: all 1s → fair = Σ prices (sum, not mean).
//  If the rulebook says "ETF = mean(constituents)", set ETF_EDGE_CENTS
//  relative to the sum scale, or divide fair by N in the arb function.
// ════════════════════════════════════════════════════════════════════
struct ETFDef {
    std::string ticker;
    std::vector<std::string> constituents;
    std::vector<int>         quantities;   // per-constituent units per 1 ETF share
    std::vector<std::string> hedge_venues;
};

static const std::vector<ETFDef> ETFS = {
    // 6-constituent ETFs — adjust quantities to match rulebook
    {"ETFA",  {"NGUP","OIT","KTST","FSR","JZRO","XFR"},  {1,1,1,1,1,1}, {"ZSE"}},
    {"ETFB",  {"KOTD","INA","HT","JNAF","DLKV","DDJH"},  {1,1,1,1,1,1}, {"ZSE"}},
    // 3-constituent subsets
    {"ETFA3", {"NGUP","KTST","XFR"},                     {1,1,1},        {"ZSE","NYSE"}},
    {"ETFB3", {"KOTD","INA","DLKV"},                     {1,1,1},        {"ZSE","NASDAQ","HKEX"}},
    // Safe-haven
    {"ETFSH", {"GOLD","XAG"},                            {1,1},          {"ZSE","Euronext","JPX"}},
};
...
public:
    ...
    void reset_segment() {
        { std::lock_guard lk(sh_mtx_);      sh_state_.clear(); }
        { std::lock_guard lk(etf_cd_mtx_);  last_etf_.clear(); }
        { std::lock_guard lk(secpair_mtx_); secpair_state_.clear(); }
        { std::lock_guard lk(xvenue_mtx_);  last_xvenue_.clear(); }
    }
...
// ─── ETF arbitrage — PICNIC_BASKET style ─────────────────────────────
//
// fair_value(1 ETF) = Σ(quantities[i] × mid_i)
//
// Signal:
//   ETF ask  <  fair − EDGE  →  BUY K ETFs + SELL quantities[i]*K of each
//   ETF bid  >  fair + EDGE  →  SELL K ETFs + BUY quantities[i]*K of each
//
// where K = ETF_HEDGE_UNIT.
//
// Spread at entry is at least EDGE cents per ETF share.  As the spread
// collapses below EXIT_CENTS the position can be closed by the inverse
// signal on the next tick (IOC orders, so no resting positions linger).
void Strategy::etf_arb(const std::string& exch) {
    ...
    for (const auto& etf : ETFS) {
        ...
        // ── Compute fair value (PICNIC_BASKET weighted sum) ──────────
        struct ConstSnap {
            std::string inst;
            int bid, ask, bid_qty, ask_qty;
            int qty;   // units of this constituent per 1 ETF share
        };
        ...
        for (int i = 0; i < N; ++i) {
            std::string ci = exch + "-" + etf.constituents[i];
            auto bk = state_.get(exch, ci);
            if (!bk) { all_ok = false; break; }
            auto m  = bk->mid();
            auto b  = bk->best_bid();
            auto a  = bk->best_ask();
            auto bq = bk->best_bid_qty();
            auto aq = bk->best_ask_qty();
            if (!m || !b || !a) { all_ok = false; break; }

            int q = etf.quantities[i];
            fair += q * (*m);   // ← weighted, not divided by N
            snaps.push_back({ci, *b, *a, bq.value_or(0), aq.value_or(0), q});
        }
        if (!all_ok) continue;
        ...
        // ── Sizing: K ETF shares, quantities[i]*K per constituent ────
        int K       = Params::ETF_HEDGE_UNIT;
        int etf_qty = K;

        // Position cap on ETF side
        ...
        // Constituent liquidity (each snap needs quantities[i]*K on opposite side)
        bool liq_ok = true;
        for (auto& s : snaps) {
            int need = s.qty * K;
            int avail = buy_etf ? s.bid_qty : s.ask_qty;
            if (avail < need) { liq_ok = false; break; }
        }
        if (!liq_ok) continue;
        ...
        if (buy_etf) {
            ...
            for (auto& s : snaps)
                if (!can_sell(c, s.inst, s.qty * K)) { liq_ok = false; break; }
        } else {
            if (!can_sell(c, etf_inst, etf_qty)) continue;
            for (auto& s : snaps)
                if (!can_buy(c, s.inst, s.qty * K, s.ask)) { liq_ok = false; break; }
        }
        if (!liq_ok) continue;

        // Reserve cooldown before firing
        {
            std::lock_guard lk(etf_cd_mtx_);
        ...
        if (buy_etf) {
            ...
            for (auto& s : snaps)
                c.place_ioc(s.inst, "ask", s.bid, s.qty * K);
        } else {
            c.place_ioc(etf_inst, "ask", *etf_bid, etf_qty);
            for (auto& s : snaps)
                c.place_ioc(s.inst, "bid", s.ask, s.qty * K);
        }

        std::cout << "[" << exch << " etf-" << etf.ticker << "] "
                  << (buy_etf ? "BUY" : "SELL") << " " << etf_qty
                  << " ETF @" << (buy_etf ? *etf_ask : *etf_bid)
                  << " fair=" << fair_int << " edge=" << edge << "c | hedge:";
        for (auto& s : snaps)
            std::cout << " " << s.inst << "x" << s.qty * K;
        std::cout << "\n";
    }
}

// ─── Safe-haven spread ───────────────────────────────────────────────
// spread = market_etf_mid − safe_etf_mid  (mean-reverting around rolling μ)
// z-score triggers long/short/flat transitions.
// FIX vs sigbot5: state is copied out while locked before pointer use.
void Strategy::safehaven_spread(const std::string& exch) {
    ...
    for (const auto& sh : SH_SPREADS) {
        ...
        // ── Update history + snapshot state (all under one lock) ─────
        int    prev_target, cur_target;
        int64_t stop_until;
        std::vector<double> snap;
        int64_t last_rebal;
        {
            std::lock_guard lk(sh_mtx_);
            auto& st = sh_state_[sh.name];
            ...
            snap        = st.spread_hist;
            prev_target = st.target_state;
            stop_until  = st.stop_until_ms;
            last_rebal  = st.last_rebal_ms;
        ...
        bool tripped = false;
        if (std::abs(z) >= stop) {
            if (cur_target != 0) { cur_target = 0; tripped = true; }
            stop_until = now + Params::SH_STOP_COOLDOWN_MS;
        }
        bool in_cooldown = now < stop_until;
        if (!tripped) {
            if      (z >= entry  && !in_cooldown)   cur_target = -1;
            else if (z <= -entry && !in_cooldown)   cur_target = +1;
            else if (std::abs(z) <= exit_)          cur_target =  0;
        }

        // ── Persist updated state ────────────────────────────────────
        bool changed = (cur_target != prev_target);
        bool rebal_ok = false;
        {
            std::lock_guard lk(sh_mtx_);
            auto& st = sh_state_[sh.name];
            st.stop_until_ms = stop_until;
            if (changed) st.target_state = cur_target;
            if (changed && now - st.last_rebal_ms >= Params::SH_REBAL_MS) {
        ...
        std::cout << "[" << exch << " sh-" << sh.name << "] z=" << z
                  << " (μ=" << mean << ", σ=" << sd << ")"
                  << " state " << prev_target << " → " << cur_target
                  << (tripped ? " [STOP]" : "") << "\n";
        if (!rebal_ok) continue;

        // ── Execute legs ─────────────────────────────────────────────
        auto& c = *conns_.at(exch);
        struct Leg { std::string inst; int sign; };
        std::vector<Leg> legs = {{m_inst, +1}, {s_inst, -1}};
        for (auto& L : legs) {
            int cur = c.position_of(L.inst);
            int desired = cur_target * L.sign * Params::SH_SIZE;
...
// ─── Sector pair z-arb ──────────────────────────────────────────────
// FIX vs sigbot5: state pointer replaced with value copy to avoid
// using a raw pointer after releasing the mutex.
void Strategy::sector_pair_zarb(const std::string& exch) {
    ...
    for (const auto& sec : SECTORS_DEF) {
        std::vector<std::string> peers, insts, keys;
        ...
        // Push current mids into rolling history
        {
        ...
        for (int i = 0; i < N; ++i) {
            // Correlation-weighted peer mean
            double peer_mean;
            ...
            if (have_corr) {
                ...
                for (int j = 0; j < N; ++j) {
                    ...
                    double corr = pearson_returns(mh[i], mh[j]);
                    double wj = std::max(0.0, corr);
                    w[j] = wj; wsum += wj;
                }
                if (have_corr && wsum > 1e-9) {
                    double s = 0.0;
                    for (int j = 0; j < N; ++j)
                        if (j != i) s += (w[j]/wsum) * mids[j];
                    peer_mean = s;
                } else {
                    have_corr = false;
                }
            ...
            // ── Update resid history + snapshot state (all under lock) ──
            int    prev_target, cur_target;
            int64_t stop_until;
            std::vector<double> snap;
            bool enough;
            ...
            if (std::abs(z) >= stop) {
                if (cur_target != 0) { cur_target = 0; tripped = true; }
                stop_until = now + Params::SECPAIR_STOP_COOLDOWN_MS;
            }
            bool in_cooldown = now < stop_until;
            if (!tripped) {
                if      (z >= entry  && !in_cooldown)  cur_target = -1;
            ...
            bool rebal_ok = false;
            {
                ...
                st.stop_until_ms = stop_until;
                if (changed) st.target_state = cur_target;
                if (changed && now - st.last_rebal_ms >= Params::SECPAIR_REBAL_MS) {
                    st.last_rebal_ms = now;
                    rebal_ok = true;
                }
            }
            ...
            if (!changed) continue;
            std::cout << "[" << exch << " sec-" << sec.name << "/" << peers[i]
                      << "] z=" << z << " (μ=" << mean << ", σ=" << sd
                      << ", N=" << N << ", corr=" << (have_corr ? "rolling" : "eq-w") << ")"
                      << " state " << prev_target << " → " << cur_target
                      << (tripped ? " [STOP]" : "") << "\n";
            if (!rebal_ok) continue;
            ...
            for (auto& mv : moves) {
                int cur_pos = c.position_of(mv.inst);
                int diff    = mv.desired - cur_pos;
                if (diff == 0) continue;
        ...
        while (running_) {
            ...
            while (running_ && c.connected()) {
                ...
                if (type == "market_data_update") {
                    ...
                    std::cout << "[" << name << "] end_of_round; reconnecting\n";
                    break;
                } else if (type == "error") {
...
int main() {
    ...
    std::cout << "AlgoTrade 2026 bot [sigbot6] — exchanges:";
