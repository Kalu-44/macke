 ...
 *   5. ETF-pair basis arb (ETFA ↔ ETFA3, ETFB ↔ ETFB3 within a venue)
 *   6. Market making (passive limit quotes inside spread on stable
 *      instruments — captures bid-ask spread without paying it)
 *
 * Build:
...
// ════════════════════════════════════════════════════════════════════
struct Params {
    // Risk caps — match exchange floors (max long=2000, short=-200, cash=-$50k)
    static constexpr int  MAX_LONG    = 2000;
    ...
    static constexpr int  ETF_EDGE_CENTS    = 12;    // entry threshold
    static constexpr int  ETF_EXIT_CENTS    = 5;     // unwind when mispricing collapses
    static constexpr int  ETF_MAX_K         = 400;   // hard cap on hedge unit per fire
    static constexpr int  ETF_POS_CAP       = 1800;  // max ETF qty per (etf, venue)
    static constexpr int  ETF_COOLDOWN_MS   = 60;    // per (etf, venue)
    ...
    static constexpr int  SECPAIR_POS_CAP          = 200;   // per ticker per venue (limited by short floor)
    ...
    static constexpr int  XVENUE_SIZE          = 2000; // per-fire cap; book qty + headroom decide
    static constexpr int  XVENUE_POS_CAP       = 1800; // long cap (short side limited by MAX_SHORT)
    static constexpr int  XVENUE_COOLDOWN_MS   = 10;   // per ticker

    // ─── ETF-pair basis arb (ETFA ↔ ETFA3, ETFB ↔ ETFB3 same-venue) ───
    // basis = ETF3_mid - ratio*ETF_mid (ratio=3 because subset basket of 3
    // constituents vs full basket of 6 → nominal ratio of underlying value).
    // Z-score basis vs rolling 60s window. Mean-reversion in 0.4-3.0s.
    // When z>0 (etf3 rich): SHORT etf3 + LONG ratio*etf  (target=-1).
    // When z<0 (etf3 cheap): LONG etf3 + SHORT ratio*etf (target=+1).
    static constexpr int  ETFPAIR_HISTORY          = 600;  // ~60s @ 100ms
    static constexpr int  ETFPAIR_MIN_SAMPLES      = 200;
    static constexpr int  ETFPAIR_ENTRY_Z100       = 250;
    static constexpr int  ETFPAIR_EXIT_Z100        = 50;
    static constexpr int  ETFPAIR_STOP_Z100        = 600;
    static constexpr int  ETFPAIR_STOP_COOLDOWN_MS = 30'000;
    static constexpr int  ETFPAIR_SIZE             = 10;   // qty of etf3 per layer
    static constexpr int  ETFPAIR_POS_CAP          = 60;   // |etf3 pos| cap (etf hedge up to 180)
    static constexpr int  ETFPAIR_REBAL_MS         = 250;

    // ─── Market making (passive limit quotes) ────────────────────
    // For each MM target instrument: post a limit BID at (best_bid + 1c)
    // and a limit ASK at (best_ask - 1c), with short expiry so stale
    // quotes auto-cancel. Skew quotes against current inventory so we
    // bleed back to flat naturally. We capture spread - 2 ticks per
    // round-trip on each fill we're hit on.
    static constexpr int  MM_SIZE              = 30;     // qty per quote
    static constexpr int  MM_MIN_SPREAD_CENTS  = 4;      // skip if internal spread too tight
    static constexpr int  MM_REPOST_MS         = 150;    // refresh per (inst, side)
    static constexpr int  MM_QUOTE_TTL_MS      = 600;    // limit order expiry
    static constexpr int  MM_POS_CAP           = 100;    // |inventory| cap before disabling that side
    static constexpr int  MM_INVENT_SKEW_CENTS = 2;      // shift bid/ask against inventory per 50 units

    // Book imbalance flow skew (proxy for aggressor flow):
    //   imbalance = (bid_qty - ask_qty) / (bid_qty + ask_qty)
    //   imbalance > +THR → upward pressure → skip ASK (don't sell into rising mid)
    //   imbalance < -THR → downward pressure → skip BID (don't buy into falling mid)
    // Threshold tuned to fire only on strong signals (~70/30 imbalance).
    static constexpr int  MM_IMBAL_SKIP_PCT    = 40;     // |imbalance%| ≥ this → skip the bad side
    static constexpr int  MM_IMBAL_MIN_QTY     = 20;     // require some depth to trust signal

    // ─── Lead-follow xvenue arb (predictive xvenue using lead/lag) ──
    // From data11 lead/lag analysis: HKEX leads ZSE on ETFB3 with
    // corr=0.61 at lag~1000ms. When the lead venue's mid moves
    // significantly recently and the follower's mid hasn't caught up,
    // fire paired IOC (buy follower / sell leader if leader moved up,
    // and reverse if leader moved down). Hedged → market-neutral.
    static constexpr int  LL_LOOKBACK_MS       = 700;     // window for leader mid change
    static constexpr int  LL_LEADER_MOVE_CENTS = 8;       // min |Δleader_mid| to trigger
    static constexpr int  LL_FOLLOWER_LAG_MAX  = 6;       // |Δfollower_mid| ≤ this means stale
    static constexpr int  LL_MIN_EDGE_CENTS    = 3;       // implied edge after move
    static constexpr int  LL_SIZE              = 60;
    static constexpr int  LL_POS_CAP           = 400;
    static constexpr int  LL_COOLDOWN_MS       = 100;     // per pair

    // ─── Per-venue circuit breaker ──────────────────────────────────
    // If a venue's cash drops below CB_TRIP_CENTS, disable directional /
    // spread strategies on that venue (sector_pair_zarb, safehaven_spread,
    // etfpair_basis_arb, market_make). Keep delta-neutral strategies
    // running (etf_arb is fully hedged; xvenue_arb is paired IOC).
    // Re-enable when cash recovers above CB_RESET_CENTS.
    static constexpr long CB_TRIP_CENTS   = -2'000'000;  // -$20k
    static constexpr long CB_RESET_CENTS  = -800'000;    // -$8k
};
...
// ══════════════════════════════════════════════════════════════════
//  ETF-pair basis arb — same-venue ETFA↔ETFA3 and ETFB↔ETFB3
//  Calibrated empirically: basis z-score mean-reverts (HL 0.4–3.0 s).
// ══════════════════════════════════════════════════════════════════
struct ETFPair {
    std::string name;     // e.g. "A_A3"
    std::string venue;
    std::string etf;      // ETFA or ETFB (base)
    std::string etf3;     // ETFA3 or ETFB3 (3-constituent subset)
    int ratio;            // basis = etf3 - ratio*etf
};

static const std::vector<ETFPair> ETF_PAIRS = {
    {"A_A3", "NYSE",   "ETFA", "ETFA3", 3},
    {"A_A3", "ZSE",    "ETFA", "ETFA3", 3},
    {"B_B3", "NASDAQ", "ETFB", "ETFB3", 3},
    {"B_B3", "HKEX",   "ETFB", "ETFB3", 3},
    {"B_B3", "ZSE",    "ETFB", "ETFB3", 3},
};

// ══════════════════════════════════════════════════════════════════
//  Market-making targets — stable, mean-reverting instruments where
//  capturing the bid-ask spread is profitable. Mostly ETFs and metals.
// ══════════════════════════════════════════════════════════════════
struct MMTarget { std::string venue; std::string ticker; };
static const std::vector<MMTarget> MM_TARGETS = {
    // ─── ETFs / metals (good spreads, mean-reverting) ──────────────
    // ZSE has all ETFs and metals locally
    {"ZSE", "ETFA"},  {"ZSE", "ETFB"},  {"ZSE", "ETFSH"},
    {"ZSE", "ETFA3"}, {"ZSE", "ETFB3"},
    {"ZSE", "GOLD"},  {"ZSE", "XAG"},
    // Other venues with ETFs
    {"NYSE",     "ETFA"},  {"NYSE",     "ETFA3"},
    {"NASDAQ",   "ETFB"},  {"NASDAQ",   "ETFB3"},
    {"HKEX",     "ETFB3"},                              // dropped HKEX-ETFB (spr 3c)
    {"Euronext", "GOLD"},                               // dropped Euronext-ETFSH (spr 3c)
    {"JPX",      "GOLD"}, {"JPX", "XAG"},               // dropped JPX-ETFSH (spr 3c)

    // ─── Stocks with wide spreads + heavy trade flow (data11 analysis) ──
    // KOTD: 9c median spread, 55-64 trades/sec across venues
    {"HKEX","KOTD"}, {"NASDAQ","KOTD"}, {"LSE","KOTD"}, {"ZSE","KOTD"}, {"Euronext","KOTD"},
    // MDKA: 8c spread, 64-68 tr/s
    {"HKEX","MDKA"}, {"NYSE","MDKA"},
    // KTST: 7-8c spread, 55-67 tr/s
    {"TMX","KTST"}, {"NYSE","KTST"}, {"ZSE","KTST"}, {"JPX","KTST"}, {"NASDAQ","KTST"},
    // NGUP: 6c spread, 63-71 tr/s
    {"NYSE","NGUP"}, {"NASDAQ","NGUP"}, {"TMX","NGUP"},
    // HT: 6c spread, 64-70 tr/s
    {"SSE","HT"}, {"JPX","HT"}, {"NASDAQ","HT"},
    // INA: 6c spread, 64-67 tr/s
    {"NASDAQ","INA"}, {"Euronext","INA"},
    // FSR: 6c spread, 63-71 tr/s
    {"SSE","FSR"}, {"LSE","FSR"},
};

// ══════════════════════════════════════════════════════════════════
//  Lead-follow pairs (from data11 lead/lag analysis).
//  leader_venue's mid moves first; follower_venue catches up after lag_ms.
//  Only listed pairs with |corr|≥0.20 and lag≥500ms.
// ══════════════════════════════════════════════════════════════════
struct LeadPair {
    std::string ticker;
    std::string leader_venue;
    std::string follower_venue;
    int  lag_ms;
};
static const std::vector<LeadPair> LEAD_PAIRS = {
    {"ETFB3", "HKEX",     "ZSE",      1000},  // corr 0.61
    {"SIMP",  "NSE",      "ZSE",       900},  // corr 0.33
    {"ZABA",  "SSE",      "LSE",       900},  // corr 0.29
    {"ZITO",  "Euronext", "NSE",       700},  // corr 0.21
};
...
public:
    ...
    bool place_limit(const std::string& inst, const std::string& side, int price, int qty, int ttl_ms) {
        bool ok = send({
            {"type","add_order"}, {"user_request_id", next_rid()},
            {"instrument_id", inst}, {"price", price},
...
public:
    ...
    void reset_segment() {
        ...
        last_xvenue_.clear();
        std::lock_guard lk5(etfpair_mtx_);
        etfpair_state_.clear();
        std::lock_guard lk6(mm_mtx_);
        last_mm_.clear();
        std::lock_guard lk7(ll_mtx_);
        ll_state_.clear();
    }
...
private:
    ...
    void etfpair_basis_arb(const std::string& exch);
    void market_make(const std::string& exch);
    void xvenue_arb();
    void lead_follow_arb();
    ...
    // ETF-pair basis state per "venue|name" key
    struct ETFPairState {
        std::vector<double> basis_hist;
        int64_t last_rebal_ms = 0;
        int target_state = 0;
        int64_t stop_until_ms = 0;
    };
    std::unordered_map<std::string, ETFPairState> etfpair_state_;
    std::mutex etfpair_mtx_;

    // Market-making per (venue, inst, side) repost cooldown
    std::unordered_map<std::string,int64_t> last_mm_;
    std::mutex mm_mtx_;

    // Per-venue circuit breaker state (true = tripped, directional strats off)
    std::unordered_map<std::string,bool> cb_tripped_;
    std::mutex cb_mtx_;
    bool cb_ok(const std::string& exch) {
        auto it = conns_.find(exch);
        if (it == conns_.end()) return true;
        long cash = it->second->cash_cents;
        std::lock_guard lk(cb_mtx_);
        bool& tripped = cb_tripped_[exch];
        if (!tripped && cash <= Params::CB_TRIP_CENTS) {
            tripped = true;
            std::cout << "[CB " << exch << "] TRIPPED cash=$" << cash/100 << "\n";
        } else if (tripped && cash >= Params::CB_RESET_CENTS) {
            tripped = false;
            std::cout << "[CB " << exch << "] RESET cash=$" << cash/100 << "\n";
        }
        return !tripped;
    }
    ...
    // Lead-follow state: rolling history of leader mid per pair.
    struct LLState {
        // (timestamp_ms, mid_cents) ring of recent leader mids
        std::vector<std::pair<int64_t,int>> leader_hist;
        std::vector<std::pair<int64_t,int>> follower_hist;
        int64_t last_fire_ms = 0;
    };
    std::unordered_map<std::string, LLState> ll_state_;  // key = "ticker|leader|follower"
    std::mutex ll_mtx_;
};
...
void Strategy::on_market_data(const std::string& exch) {
    ...
    try {
        // Always-safe (delta-neutral or paired): run even if venue is bleeding.
        etf_arb(exch);
        xvenue_arb();
        lead_follow_arb();
        // Directional / spread strategies: gated by per-venue circuit breaker.
        if (cb_ok(exch)) {
            safehaven_spread(exch);
            sector_pair_zarb(exch);
            etfpair_basis_arb(exch);
            market_make(exch);
        }
    } catch (const std::exception& e) {
...
void Strategy::etf_arb(const std::string& exch) {
    ...
    for (const auto& etf : ETFS) {
        ...
        // ── Compute MAX hedge unit K ────────────────────────────────
        // ETF leg: need N*K shares of ETF on the consuming side.
        // Each constituent leg: need K shares on the opposite side.
        // K is capped by:
        //   1. ETF top-of-book qty / N
        //   2. min over constituents of their top-of-book qty (opposite side)
        //   3. position headroom on ETF: (POS_CAP ± etf_pos) / N
        //   4. ETF_MAX_K hard cap
        int etf_side_qty = buy_etf ? etf_ask_qty.value_or(0)
                                   : etf_bid_qty.value_or(0);
        int K = etf_side_qty / N;
        for (auto& s : snaps) {
            int q = buy_etf ? s.bid_qty : s.ask_qty;
            K = std::min(K, q);
        }
        int etf_headroom = buy_etf ? (Params::ETF_POS_CAP - etf_pos)
                                   : (Params::ETF_POS_CAP + etf_pos);
        K = std::min(K, etf_headroom / N);
        K = std::min(K, Params::ETF_MAX_K);
        if (K <= 0) continue;
        ...
        // Position cap (re-check with computed etf_qty)
        ...
        if (buy_etf) {
            ...
            for (auto& s : snaps)
                if (!can_sell(c, s.inst, leg_qty)) { ok = false; break; }
        } else {
            if (!can_sell(c, etf_inst, etf_qty)) continue;
            for (auto& s : snaps)
                if (!can_buy(c, s.inst, leg_qty, s.ask)) { ok = false; break; }
        }
        if (!ok) continue;
...
// ─── ETF-pair basis arb (ETFA↔ETFA3, ETFB↔ETFB3 within a venue) ──────
// basis = etf3_mid - ratio * etf_mid.   z = (basis - rolling μ)/σ.
// z>0 (etf3 rich vs ratio*etf): SHORT etf3 + LONG ratio*etf  (target=-1)
// z<0 (etf3 cheap):             LONG etf3 + SHORT ratio*etf  (target=+1)
void Strategy::etfpair_basis_arb(const std::string& exch) {
    if (!state_.warmup_done()) return;
    int64_t now = now_ms();

    for (const auto& p : ETF_PAIRS) {
        if (p.venue != exch) continue;

        std::string etf_inst  = exch + "-" + p.etf;
        std::string etf3_inst = exch + "-" + p.etf3;
        auto bk_etf  = state_.get(exch, etf_inst);
        auto bk_etf3 = state_.get(exch, etf3_inst);
        if (!bk_etf || !bk_etf3) continue;
        auto m_etf  = bk_etf->mid();
        auto m_etf3 = bk_etf3->mid();
        if (!m_etf || !m_etf3) continue;

        double basis = *m_etf3 - (double)p.ratio * (*m_etf);

        std::string key = exch + "|" + p.name;
        ETFPairState* st;
        {
            std::lock_guard lk(etfpair_mtx_);
            st = &etfpair_state_[key];
            st->basis_hist.push_back(basis);
            if ((int)st->basis_hist.size() > Params::ETFPAIR_HISTORY)
                st->basis_hist.erase(st->basis_hist.begin());
        }

        if ((int)st->basis_hist.size() < Params::ETFPAIR_MIN_SAMPLES) continue;
        ...
        double sd = std::sqrt(var);
        if (sd < 1e-6) continue;
        double z = (basis - mean) / sd;

        const double entry = Params::ETFPAIR_ENTRY_Z100 / 100.0;
        const double exit_ = Params::ETFPAIR_EXIT_Z100  / 100.0;
        const double stop  = Params::ETFPAIR_STOP_Z100  / 100.0;
        ...
        if (std::abs(z) >= stop) {
            if (prev != 0) { target = 0; tripped = true; }
            st->stop_until_ms = now + Params::ETFPAIR_STOP_COOLDOWN_MS;
        }
        bool in_cooldown = now < st->stop_until_ms;
        if (!tripped) {
            if (z >= entry && !in_cooldown)        target = -1;  // etf3 rich
            else if (z <= -entry && !in_cooldown)  target = +1;  // etf3 cheap
            else if (std::abs(z) <= exit_)         target = 0;
        }

        if (target == prev) continue;
        st->target_state = target;
        std::cout << "[" << exch << " etfpair-" << p.name << "] z=" << z
                  << " (μ=" << mean << ", σ=" << sd << ")"
                  << " state " << prev << " → " << target
                  << (tripped ? " [STOP]" : "") << "\n";
        ...
        {
            std::lock_guard lk(etfpair_mtx_);
            if (now - st->last_rebal_ms < Params::ETFPAIR_REBAL_MS) continue;
            st->last_rebal_ms = now;
        }

        // Execute legs:
        //  target=+1  → LONG  etf3 (size)   + SHORT etf (ratio*size)
        //  target=-1  → SHORT etf3 (size)   + LONG  etf (ratio*size)
        //  target=0   → flatten both
        auto& c = *conns_.at(exch);
        struct Leg { std::string inst; int sign; int size; };
        std::vector<Leg> legs = {
            {etf3_inst, +1, Params::ETFPAIR_SIZE},
            {etf_inst,  -1, Params::ETFPAIR_SIZE * p.ratio},
        };
        for (auto& L : legs) {
            int cur = c.position_of(L.inst);
            int desired = target * L.sign * L.size;
            int cap = Params::ETFPAIR_POS_CAP * (L.size / Params::ETFPAIR_SIZE);
            ...
            if (diff > 0) {
                if (!bk->best_ask() || !bk->best_ask_qty()) continue;
                int qty = std::min({diff, *bk->best_ask_qty(), 100});
                if (qty <= 0 || !can_buy(c, L.inst, qty, *bk->best_ask())) continue;
                c.place_ioc(L.inst, "bid", *bk->best_ask(), qty);
            } else {
                int need = -diff;
                if (!bk->best_bid() || !bk->best_bid_qty()) continue;
                int qty = std::min({need, *bk->best_bid_qty(), 100});
                if (qty <= 0 || !can_sell(c, L.inst, qty)) continue;
                c.place_ioc(L.inst, "ask", *bk->best_bid(), qty);
            }
        }
...
// ─── Market making (passive limit quotes inside spread) ────────────
// For each MM target listed on this venue, post limit BID at
// (best_bid + 1) and limit ASK at (best_ask - 1) with short expiry.
// Quotes auto-expire so we don't need to track/cancel. Inventory skew:
// if long, lower bid more (don't want to add) and lower ask more
// (eager to sell); if short, do the inverse.
void Strategy::market_make(const std::string& exch) {
    if (!state_.warmup_done()) return;
    int64_t now = now_ms();

    for (const auto& mm : MM_TARGETS) {
        if (mm.venue != exch) continue;
        std::string inst = exch + "-" + mm.ticker;
        auto bk = state_.get(exch, inst);
        if (!bk) continue;
        auto bb = bk->best_bid(); auto ba = bk->best_ask();
        if (!bb || !ba) continue;
        int spread = *ba - *bb;
        if (spread < Params::MM_MIN_SPREAD_CENTS) continue;

        auto& c = *conns_.at(exch);
        int pos = c.position_of(inst);

        // Inventory skew: shift quotes against current position so we
        // bleed back toward flat. skew_cents = MM_INVENT_SKEW_CENTS * (pos / 50)
        int skew = (pos / 50) * Params::MM_INVENT_SKEW_CENTS;

        // Improve by 1 tick inside the spread.
        int my_bid = *bb + 1 - skew;   // long pos → skew>0 → lower bid (less eager)
        int my_ask = *ba - 1 - skew;   // long pos → skew>0 → lower ask (more eager)

        // Validate against book to avoid crossing
        if (my_bid >= *ba) my_bid = *ba - 1;
        if (my_ask <= *bb) my_ask = *bb + 1;
        if (my_ask <= my_bid) continue;

        // Book-imbalance flow skew: skip the side price is moving toward.
        // imbalance > 0  → bids dominate → mid likely to rise → skip ASK
        // imbalance < 0  → asks dominate → mid likely to fall → skip BID
        bool skip_bid = false, skip_ask = false;
        int bbq = bk->best_bid_qty().value_or(0);
        int baq = bk->best_ask_qty().value_or(0);
        if (bbq + baq >= Params::MM_IMBAL_MIN_QTY) {
            int imbal_pct = (100 * (bbq - baq)) / (bbq + baq);
            if (imbal_pct >= Params::MM_IMBAL_SKIP_PCT) skip_ask = true;
            if (imbal_pct <= -Params::MM_IMBAL_SKIP_PCT) skip_bid = true;
        }

        // BID side
        if (!skip_bid && pos < Params::MM_POS_CAP) {
            std::string key_b = exch + "|" + mm.ticker + "|b";
            bool fire = false;
            {
                std::lock_guard lk(mm_mtx_);
                auto it = last_mm_.find(key_b);
                if (it == last_mm_.end() || now - it->second >= Params::MM_REPOST_MS) {
                    last_mm_[key_b] = now;
                    fire = true;
                }
            }
            if (fire && can_buy(c, inst, Params::MM_SIZE, my_bid)) {
                c.place_limit(inst, "bid", my_bid, Params::MM_SIZE, Params::MM_QUOTE_TTL_MS);
                std::cout << "[" << exch << " mm-" << mm.ticker << "] bid "
                          << Params::MM_SIZE << "@" << my_bid
                          << " (book " << *bb << "/" << *ba << " spr=" << spread
                          << "c) pos=" << pos << "\n";
            }
        }
        // ASK side
        if (!skip_ask && pos > -Params::MM_POS_CAP) {
            std::string key_a = exch + "|" + mm.ticker + "|a";
            bool fire = false;
            {
                std::lock_guard lk(mm_mtx_);
                auto it = last_mm_.find(key_a);
                if (it == last_mm_.end() || now - it->second >= Params::MM_REPOST_MS) {
                    last_mm_[key_a] = now;
                    fire = true;
                }
            }
            if (fire && can_sell(c, inst, Params::MM_SIZE)) {
                c.place_limit(inst, "ask", my_ask, Params::MM_SIZE, Params::MM_QUOTE_TTL_MS);
                std::cout << "[" << exch << " mm-" << mm.ticker << "] ask "
                          << Params::MM_SIZE << "@" << my_ask
                          << " (book " << *bb << "/" << *ba << " spr=" << spread
                          << "c) pos=" << pos << "\n";
            }
        }
...
void Strategy::sector_pair_zarb(const std::string& exch) {
    ...
    for (const auto& sec : SECTORS_DEF) {
        ...
        for (int i = 0; i < N; ++i) {
            ...
            // weight: stock_i = +(N-1)*scale, peer = -1*scale; scaled by target.
            // scale grows with |z|: scale = clamp(|z|/entry, 1, POS_CAP/(N-1))
            // Each leg capped at SECPAIR_POS_CAP.
            auto& c = *conns_.at(exch);
            struct Move { std::string inst; int desired; };
            std::vector<Move> moves;

            int max_scale = std::max(1, Params::SECPAIR_POS_CAP / std::max(1, N - 1));
            int scale = (target == 0) ? 1
                        : std::clamp((int)std::lround(std::abs(z) / entry), 1, max_scale);

            int desired_i = target * (N - 1) * scale;
            if (desired_i >  Params::SECPAIR_POS_CAP) desired_i =  Params::SECPAIR_POS_CAP;
            if (desired_i < -Params::SECPAIR_POS_CAP) desired_i = -Params::SECPAIR_POS_CAP;
            moves.push_back({insts[i], desired_i});
            for (int j = 0; j < N; ++j) {
                if (j == i) continue;
                int dp = target * (-1) * scale;
...
// ─── Lead-follow xvenue arb ──────────────────────────────────────────
// For each (ticker, leader_venue, follower_venue) pair from the
// data11 lead/lag analysis, watch the leader's mid history. If the
// leader's mid changed by ≥LL_LEADER_MOVE_CENTS in the last
// LL_LOOKBACK_MS while the follower's mid barely moved, fire a paired
// IOC: take the side on the follower that the leader's move predicts,
// hedged by an opposite IOC on the leader. Market-neutral; profit
// comes from the follower's catch-up move.
void Strategy::lead_follow_arb() {
    if (!state_.warmup_done()) return;
    int64_t t = now_ms();

    for (const auto& lp : LEAD_PAIRS) {
        std::string lead_inst = lp.leader_venue + "-" + lp.ticker;
        std::string foll_inst = lp.follower_venue + "-" + lp.ticker;

        auto lb = state_.get(lp.leader_venue, lead_inst);
        auto fb = state_.get(lp.follower_venue, foll_inst);
        if (!lb || !fb) continue;
        auto lbb = lb->best_bid(); auto lba = lb->best_ask();
        auto fbb = fb->best_bid(); auto fba = fb->best_ask();
        if (!lbb || !lba || !fbb || !fba) continue;
        int lead_mid = (*lbb + *lba) / 2;
        int foll_mid = (*fbb + *fba) / 2;

        std::string key = lp.ticker + "|" + lp.leader_venue + "|" + lp.follower_venue;

        // Update history & compute leader move over lookback.
        int lead_old = lead_mid, foll_old = foll_mid;
        bool can_fire = false;
        {
            std::lock_guard lk(ll_mtx_);
            auto& st = ll_state_[key];
            st.leader_hist.emplace_back(t, lead_mid);
            st.follower_hist.emplace_back(t, foll_mid);
            // Drop entries older than 2*lookback
            int64_t cutoff = t - 2 * Params::LL_LOOKBACK_MS;
            while (!st.leader_hist.empty() && st.leader_hist.front().first < cutoff)
                st.leader_hist.erase(st.leader_hist.begin());
            while (!st.follower_hist.empty() && st.follower_hist.front().first < cutoff)
                st.follower_hist.erase(st.follower_hist.begin());

            // Find leader/follower mid at (t - LL_LOOKBACK_MS).
            int64_t target_t = t - Params::LL_LOOKBACK_MS;
            for (auto& [ts, px] : st.leader_hist)   { if (ts <= target_t) lead_old = px; }
            for (auto& [ts, px] : st.follower_hist) { if (ts <= target_t) foll_old = px; }

            // Cooldown
            if (t - st.last_fire_ms < Params::LL_COOLDOWN_MS) continue;
            can_fire = (st.leader_hist.size() > 5 && st.follower_hist.size() > 5);
            if (can_fire) st.last_fire_ms = t;  // pre-reserve; un-set on no-fire below
        }
        if (!can_fire) continue;

        int lead_delta = lead_mid - lead_old;
        int foll_delta = foll_mid - foll_old;
        if (std::abs(lead_delta) < Params::LL_LEADER_MOVE_CENTS) {
            std::lock_guard lk(ll_mtx_); ll_state_[key].last_fire_ms = 0; continue;
        }
        if (std::abs(foll_delta) > Params::LL_FOLLOWER_LAG_MAX) {
            std::lock_guard lk(ll_mtx_); ll_state_[key].last_fire_ms = 0; continue;
        }

        // Direction: leader up → follower predicted to go up → BUY follower, SELL leader.
        // Edge expected = lead_delta - foll_delta (catch-up gap).
        int expected_catchup = std::abs(lead_delta - foll_delta);
        if (expected_catchup < Params::LL_MIN_EDGE_CENTS) {
            std::lock_guard lk(ll_mtx_); ll_state_[key].last_fire_ms = 0; continue;
        }

        auto& c_lead = *conns_.at(lp.leader_venue);
        auto& c_foll = *conns_.at(lp.follower_venue);

        bool leader_up = lead_delta > 0;
        // Position caps
        int pos_lead = c_lead.position_of(lead_inst);
        int pos_foll = c_foll.position_of(foll_inst);

        int qty = Params::LL_SIZE;
        if (leader_up) {
            // BUY follower (best ask), SELL leader (best bid)
            int buy_px  = *fba;
            int sell_px = *lbb;
            int avail_b = fb->best_ask_qty().value_or(0);
            int avail_s = lb->best_bid_qty().value_or(0);
            qty = std::min({qty, avail_b, avail_s,
                            Params::LL_POS_CAP - pos_foll,
                            Params::LL_POS_CAP + pos_lead});
            if (qty <= 0) continue;
            if (!can_buy(c_foll, foll_inst, qty, buy_px)) continue;
            if (!can_sell(c_lead, lead_inst, qty)) continue;
            c_foll.place_ioc(foll_inst, "bid", buy_px,  qty);
            c_lead.place_ioc(lead_inst, "ask", sell_px, qty);
            std::cout << "[ll " << lp.ticker << "] LEADER " << lp.leader_venue
                      << " up " << lead_delta << "c, BUY " << lp.follower_venue
                      << "@" << buy_px << " SELL " << lp.leader_venue
                      << "@" << sell_px << " qty=" << qty
                      << " expCatchup=" << expected_catchup << "c\n";
        } else {
            // SELL follower, BUY leader
            int sell_px = *fbb;
            int buy_px  = *lba;
            int avail_s = fb->best_bid_qty().value_or(0);
            int avail_b = lb->best_ask_qty().value_or(0);
            qty = std::min({qty, avail_b, avail_s,
                            Params::LL_POS_CAP - pos_lead,
                            Params::LL_POS_CAP + pos_foll});
            if (qty <= 0) continue;
            if (!can_buy(c_lead, lead_inst, qty, buy_px)) continue;
            if (!can_sell(c_foll, foll_inst, qty)) continue;
            c_lead.place_ioc(lead_inst, "bid", buy_px,  qty);
            c_foll.place_ioc(foll_inst, "ask", sell_px, qty);
            std::cout << "[ll " << lp.ticker << "] LEADER " << lp.leader_venue
                      << " dn " << lead_delta << "c, SELL " << lp.follower_venue
                      << "@" << sell_px << " BUY " << lp.leader_venue
                      << "@" << buy_px << " qty=" << qty
                      << " expCatchup=" << expected_catchup << "c\n";
        }
    }
...
нед, 10. мај 2026. у 12:32 Đurđina Filipović <djurdjinafilipovickg@gmail.com> је написао/ла:
/*
 * AlgoTrade 2026 — Competition Bot (C++20)
 *
 * Four strategies:
...
struct Params {
    ...
    static constexpr int  ETF_EXIT_CENTS    = 5;     // unwind when mispricing collapses
    static constexpr int  ETF_MAX_K         = 200;   // hard cap on hedge unit per fire
    static constexpr int  ETF_POS_CAP       = 1500;  // max ETF qty per (etf, venue)
    ...
    // bleed back to flat naturally. We capture spread - 2 ticks per
    // round-trip on each fill we're hit on.
    static constexpr int  MM_SIZE              = 20;     // qty per quote
    static constexpr int  MM_MIN_SPREAD_CENTS  = 5;      // skip if internal spread too tight
    static constexpr int  MM_REPOST_MS         = 200;    // refresh per (inst, side)
    static constexpr int  MM_QUOTE_TTL_MS      = 600;    // limit order expiry
    static constexpr int  MM_POS_CAP           = 80;     // |inventory| cap before disabling that side
    static constexpr int  MM_INVENT_SKEW_CENTS = 2;      // shift bid/ask against inventory per 50 units
    ...
    // and reverse if leader moved down). Hedged → market-neutral.
    static constexpr int  LL_LOOKBACK_MS       = 700;     // window for leader mid change
    static constexpr int  LL_LEADER_MOVE_CENTS = 12;      // min |Δleader_mid| to trigger
    static constexpr int  LL_FOLLOWER_LAG_MAX  = 4;       // |Δfollower_mid| ≤ this means stale
    static constexpr int  LL_MIN_EDGE_CENTS    = 4;       // implied edge after move
    static constexpr int  LL_SIZE              = 25;
    static constexpr int  LL_POS_CAP           = 200;
    static constexpr int  LL_COOLDOWN_MS       = 200;     // per pair
