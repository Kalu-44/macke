...
struct Params {
    ...
    static constexpr int  LEAD_LAG_EDGE_CENTS = 10;   // ≥10c gap → fire (raised from 8)
    static constexpr int  LEAD_LAG_SIZE       = 20;   // shares per fire (lowered from 30)
    static constexpr int  LEAD_LAG_COOLDOWN_MS = 150; // per (lag, ticker) cooldown
    static constexpr int  LEAD_LAG_POS_CAP     = 60;  // |position| cap per (venue,ticker)
    static constexpr int  LEAD_LAG_EXIT_CENTS  = 2;   // exit when gap shrinks below this
...
//
// Two-phase logic per pair:
//   ENTRY: open a position when |lead_mid - lag_quote| >= EDGE_CENTS
//          and current pos within ±POS_CAP.
//   EXIT:  flatten the position once the gap has closed (≤ EXIT_CENTS),
//          regardless of which side we are on. This realises the profit
//          before the lag book can move against us.
void Strategy::lead_lag_arb(const std::string& exch) {
    ...
    for (const auto& p : LEAD_LAG_TABLE) {
        ...
        // ── EXIT: gap has closed → flatten existing position ──
        if (pos > 0 && lag_bid) {
            // We are long; lead-mid is no longer above lag-ask by EDGE → sell
            if (!lag_ask || (*lead_mid - *lag_ask) < Params::LEAD_LAG_EXIT_CENTS) {
                ...
                continue;  // skip entry on this pair this tick
            }
        }
        if (pos < 0 && lag_ask) {
            // We are short; lag-bid is no longer above lead-mid by EDGE → buy back
        ...
        // ── ENTRY: open position toward expected mean-revert direction ──
