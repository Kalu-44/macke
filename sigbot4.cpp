 ...
 *   4. Cross-venue same-ticker hedged arbitrage (xvenue, all stocks)
 *   5. CARD/SIMP super-arb (10-venue global min-ask/max-bid scan)
 *
 * Build:
...
struct Params {
    ...
    //   residual_i = price_i − mean(price_j, j≠i in same sector on venue)
    ...
    static constexpr int  SECPAIR_HISTORY          = 600;   // ~60s @ 100ms
    static constexpr int  SECPAIR_MIN_SAMPLES      = 150;   // 15s warmup of own series
    ...
    // No transaction fees → minimum 1c (any positive gap is pure profit).
    static constexpr int  XVENUE_EDGE_CENTS    = 1;
    static constexpr int  XVENUE_SIZE          = 50;
    static constexpr int  XVENUE_POS_CAP       = 150;
    static constexpr int  XVENUE_COOLDOWN_MS   = 100; // per pair+ticker

    // ─── CARD/SIMP super-arb (10-venue global min/max scan) ──────────
    // CARD and SIMP are listed on all 10 venues but priced INDEPENDENTLY
    // by each venue's MM. Empirical analysis shows huge persistent spreads
    // (mean ~$2-7, max >$70). Strategy: every tick, scan all venues, find
    // venue with cheapest ask and venue with richest bid. If their gap
    // exceeds CS_EDGE_CENTS, lift the cheap and hit the rich simultaneously.
    // Larger size + tighter edge than generic xvenue arb because the gap is
    // almost always present.
    // edge=1 cent: pure positive expectancy, only filters out fee-eating noise
    static constexpr int  CS_EDGE_CENTS      = 1;    // any positive gap counts
    static constexpr int  CS_SIZE            = 50;   // shares per leg per fire
    static constexpr int  CS_POS_CAP         = 150;  // max ABS pos per (ticker, venue)
    static constexpr int  CS_COOLDOWN_MS     = 100;  // per ticker (global)
};
...
private:
    ...
    // CARD/SIMP super-arb per-ticker cooldown
    std::unordered_map<std::string,int64_t> last_cs_;
    std::mutex cs_mtx_;
};
...
void Strategy::sector_pair_zarb(const std::string& exch) {
    ...
    for (const auto& sec : SECTORS_DEF) {
        ...
        // Sum of all mids for fast peer-mean computation
        double sum_all = 0.0;
        for (double m : mids) sum_all += m;

        for (int i = 0; i < N; ++i) {
            double peer_mean = (sum_all - mids[i]) / (N - 1);
            double resid = mids[i] - peer_mean;

            std::string key = exch + "|" + sec.name + "|" + peers[i];
            SecPairState* st;
            {
...
// ─── CARD/SIMP super-arb ─────────────────────────────────────────────
// CARD and SIMP have huge persistent cross-venue mispricings (mean spread
// $2-7, max $70+). Every tick: scan all 10 venues, find venue with cheapest
// ask (best place to BUY) and venue with richest bid (best place to SELL).
// If those came from different venues with positive gap, fire paired IOC.
// No edge gate (any positive gap is profit, since fees are negligible).
void Strategy::cardsimp_super_arb() {
    if (!state_.warmup_done()) return;

    static const std::vector<std::string> CS_TICKERS = {"CARD", "SIMP"};

    for (const auto& ticker : CS_TICKERS) {
        // Per-ticker cooldown (global, not per-pair)
        {
            std::lock_guard lk(cs_mtx_);
            int64_t t = now_ms();
            auto it = last_cs_.find(ticker);
            if (it != last_cs_.end() && t - it->second < Params::CS_COOLDOWN_MS)
                continue;
            // tentatively reserve; we restore-on-skip below if no opportunity
            last_cs_[ticker] = t;
        }

        // Scan all 10 venues for this ticker
        ...
        for (const auto& [vname, conn] : conns_) {
        ...
        int headroom_buy  = Params::CS_POS_CAP - pos_buy;
        int headroom_sell = Params::CS_POS_CAP + pos_sell;
        int qty = std::min({Params::CS_SIZE,
        ...
        std::cout << "[cs-super " << ticker << "] buy " << best_ask_v
