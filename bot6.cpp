...
static const std::vector<CointBasket> COINT_BASKETS = {
    ...
    {"B", "ZSE", {
        // TODO: fill once Joco computes Johansen vector for {KOTD,INA,HT,JNAF,DLKV,DDJH}
    }},
};
...
//   1. Read venue-local mids for each leg
//   2. spread = Σ β_i · mid_i, push into rolling history
//   3. Once we have ≥ COINT_MIN_SAMPLES, compute z = (s − μ)/σ
//   4. Determine target_state with hysteresis:
//        -1 (short spread) if z >=  entry
//        +1 (long  spread) if z <= -entry
//         0 (flat)         if |z| <= exit
//        else hold previous
//   5. If target_state changed, send IOCs to push each leg toward
//      target_pos = target_state · round(K · β_i).
void Strategy::coint_basket_trade(const std::string& exch) {
    for (const auto& cb : COINT_BASKETS) {
        ...
        auto& c = *conns_.at(cb.venue);

        // 1) Spread from venue-local mids.
        double spread = 0.0;
        bool ok = true;
        ...
        int64_t now = now_ms();
        if (now - cs->last_rebal_ms < Params::COINT_REBAL_MS) continue;
        ...
        // 4) Target state with hysteresis.
        ...
        cs->last_rebal_ms = now;

        std::cout << "[ZSE coint-" << cb.name << "] z=" << z
                  << " spread=" << spread << " (μ=" << mean << ", σ=" << sd << ")"
                  << " state " << prev << " → " << target << "\n";

        // 5) Move each leg toward target_pos.
        for (auto& [tkr, beta] : cb.legs) {
            std::string inst = cb.venue + "-" + tkr;
            int leg_units  = (int)std::lround(Params::COINT_BASKET_K * beta);
            int target_pos = target * leg_units;
