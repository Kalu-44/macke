...
static int arb_size_for(const std::string& etf) {
    if (etf == "ETFB3") return 10;
    if (etf == "ETFSH") return 15;
...
public:
   ...
   std::optional<double> consensus_mid(const std::string& tkr) const {
    ...
    for (auto& exch : listings_for(tkr)) {
        auto eit = books_.find(exch);
        if (eit == books_.end()) continue;
 
        auto iit = eit->second.find(exch + "-" + tkr);
        if (iit == eit->second.end()) continue;
 
        const Book& bk = iit->second;
        auto b = bk.best_bid();
        auto a = bk.best_ask();
        if (!b || !a || *a <= *b) continue;  // nema two-sided ili crossed book
 
        double mid    = (*b + *a) / 2.0;
        double spread = *a - *b;
 
        // Inverse-spread weighting: uži spread → veći weight.
        // Primer: spread=2c → w=0.5; spread=10c → w=0.1
        // Spreadu stavimo floor od 1c da ne delimo s nulom.
        double w = 1.0 / std::max(spread, 1.0);
        w_sum  += w;
        wm_sum += w * mid;
    }
 
    if (w_sum == 0.0) return std::nullopt;
    return wm_sum / w_sum;
}
    ...
    auto it = ETF_BASKETS.find(etf);
    if (it == ETF_BASKETS.end()) return std::nullopt;
    const auto& basket = it->second;
 
    double sum     = 0.0;
    int    n_live  = 0;
    int    n_total = (int)basket.size();
 
    for (auto& tkr : basket) {
        auto m = consensus_mid(tkr);
        if (!m) continue;  // nema quote za ovaj constituent — preskačemo
        sum += *m;
        ++n_live;
    }
 
    // Stara verzija: n_live < n_total → return nullopt (sve ili ništa).
    // Nova verzija: dozvoljavamo do 1 missing constituent.
    // Threshold: ako fali više od 1, signal je previše nesiguran.
    if (n_live == 0) return std::nullopt;
    // Dozvoli missing ako je bar 80% košarice pokriveno.
// Za ETFA(6): potrebno ≥5. Za ETFA3/ETFB3(3): potrebno ≥2 — ključna razlika.
    if (n_live < std::max(2, (int)(n_total * 0.80))) return std::nullopt;
 
    // Skaliramo sumu na pun broj konstituentnih akcija.
    // Pretpostavka: missing constituent trguje po proseku ostalih.
    // Ovo je konzervativna aproksimacija — greška je mala kada fali 1/6.
    return (sum / n_live);  // prosek živih, kao proxy za NAV po akciji
}
    ...
    //─── Gap history za etfs (etf_mid - nav) ───
    void record_gap(const std::string& etf, double etf_mid, double nav) {
    std::lock_guard lk(gap_mtx_);
    auto& h = gap_history_[etf];
    h.push_back(etf_mid - nav);
    if ((int)h.size() > GAP_HISTORY_LEN) h.erase(h.begin());
}

// Vraća rolling std gapa, ili nullopt ako nema dovoljno podataka
std::optional<double> gap_std(const std::string& etf) const {
    std::lock_guard lk(gap_mtx_);
    auto it = gap_history_.find(etf);
    if (it == gap_history_.end() || (int)it->second.size() < 30) return std::nullopt;
    const auto& h = it->second;
    double sum = 0; for (double x : h) sum += x;
    double mean = sum / h.size();
    double var  = 0; for (double x : h) var += (x-mean)*(x-mean);
    return std::sqrt(var / h.size());
}
...
private:
    ...
    mutable std::mutex gap_mtx_;
    std::unordered_map<std::string, std::vector<double>> gap_history_;
    static constexpr int GAP_HISTORY_LEN = 100;  // ~10 sekundi pri 100ms tiku
};
...
void Strategy::etf_arb(const std::string& exch) {
    ...
    for (auto& [etf, exs] : ETF_LISTINGS) {
        ...
        if (bk->best_bid() && (*bk->best_bid() - *nav) >= dyn_edge) {
            ...
            int qty = std::min(arb_size_for(etf), *bk->best_bid_qty());
            if (qty > 0 && can_sell(c, inst, qty))
                c.place_ioc(inst, "ask", *bk->best_bid(), qty);
        }
        if (bk->best_ask() && (*nav - *bk->best_ask()) >= dyn_edge) {
            // ↓↓↓ IZMENA — bilo: Params::ARB_SIZE
            int qty = std::min(arb_size_for(etf), *bk->best_ask_qty());
